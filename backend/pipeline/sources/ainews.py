...
def _collect_links(html: str) -> list[dict]:
    return [
        {"url": m.group(1), "host": _host_of(m.group(1)), "text": _strip_tags(m.group(2))}
...
_JUNK_HOSTS = {"news.smol.ai", "latent.space", "support.substack.com", "i.redd.it", "preview.redd.it"}
...
def _classify_host(host: str) -> str | None:
    ...
    if host in ("arxiv.org", "ar5iv.labs.arxiv.org"):
        return "arxiv"
    if BLOG_HOST_RX.search(host) or host.endswith(".ai") or host.endswith(".dev") or "substack.com" in host:
        return "blog"
...
def _extract_twitter_recap(html: str) -> dict | None:
    region = _slice_section(html, re.compile(r'<h1[^>]*>\s*AI Twitter Recap\s*</h1>', re.IGNORECASE))
    if not region:
        return None
    ...
    if not primary["url"]:
        return None
    bullets = [_strip_tags(m.group(1)) for m in re.finditer(r'<li>([\s\S]*?)</li>', region)][:3]
    content = " ".join(bullets) or _strip_tags(region)
    return {"title": title, "url": primary["url"], "content": content[:MAX_CONTENT_LENGTH]}
...
def _extract_reddit_recap(html: str) -> list[dict]:
    region = _slice_section(html, re.compile(r'<h1[^>]*>\s*AI Reddit Recap\s*</h1>', re.IGNORECASE))
    if not region:
        return []

    anchors = [
        {"url": m.group(1), "title": _strip_tags(m.group(2)), "start": m.start(), "end": m.end()}
    ...
    for i, anchor in enumerate(anchors):
        ...
        heading_abs = anchor["end"] + heading_rel.start() if heading_rel else len(region)
...
def fetch_ai_news() -> list[dict]:
    ...
    for item in items:
        ...
        if twitter:
            articles.append({**twitter, "published_date": item["pub_date"], "source": "AI News", "source_type": "aiNews"})
        for reddit in _extract_reddit_recap(item["content_html"]):
            articles.append({**reddit, "published_date": item["pub_date"], "source": "AI News", "source_type": "aiNews"})
